    ...
    for epoch in range(n_epochs):
        ...
        for Batch_data in tqdm_train_loader:
            ...
            if "pheme" in dataname:
              ...
              weight = th.tensor([3.0, 1.0]).to(device)#non-rumor v.s rumor
              loss = F.nll_loss(out_labels, Batch_data.y,weight=weight)#negative loss likelyhood
        ...
        for Batch_data in tqdm_test_loader:
            ...
            else:#twitter15,twitter16
              weight = th.tensor([3.0, 1.0]).to(device)#non-rumor,rumor
              val_loss = F.nll_loss(val_out, Batch_data.y,weight=weight)
        ...
        writer.add_scalars(datasetname+'iter{}_Loss{}'.format(iter,fold), {'Loss/train':np.mean(avg_loss),
                                  'Loss/validation':  np.mean(temp_val_losses)}, epoch)
        writer.add_scalars(datasetname+'iter{}_Acc{}'.format(iter,fold), {'Acc/train':np.mean(avg_acc),
                                  'Acc/validation':  np.mean(temp_val_accs)}, epoch)
        writer.flush()
...
writer = SummaryWriter('runs/'+datasetname)
savepath = 'save_model/'+datasetname
if not os.path.exists(savepath):
  os.makedirs(savepath)
with open('{}/average_performance.tsv'.format(savepath), 'w') as f:
        f.write('Iter\tFold\tTestAcc\tACC1\tPRE1\tREC1\tF1_1\tACC2\tPRE2\tREC2\tF1_2\n')
...
for iter in range(iterations):
    ...
    #print(len(fold0_x_test),len(fold0_x_train),len(fold1_x_test),len(fold1_x_train),len(fold2_x_test),len(fold2_x_train),len(fold3_x_test),len(fold3_x_train),len(fold4_x_test),len(fold4_x_train))
    train_losses, val_losses, train_accs, val_accs, accs_0, acc1_0, pre1_0, rec1_0, F1_0, acc2_0, pre2_0, rec2_0, F2_0 = train_GCN(treeDic,
                                                                                               ...
                                                                                               datasetname,
                                                                                               iter,0)
    with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
        f.write('{}\t{}\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:4f}\t{:.4f}\n'
            .format(iter, 0, acc1_0, pre1_0, rec1_0, F1_0, acc2_0, pre2_0, rec2_0, F2_0))
    ...
    with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
        f.write('{}\t{}\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:4f}\t{:.4f}\n'
            .format(iter, 1, acc1_1, pre1_1, rec1_1, F1_1, acc2_1, pre2_1, rec2_1, F2_1))
    ...
    with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
        f.write('{}\t{}\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:4f}\t{:.4f}\n'
            .format(iter, 2, acc1_2, pre1_2, rec1_2, F1_2, acc2_2, pre2_2, rec2_2, F2_2))
    ...
    with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
        f.write('{}\t{}\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:4f}\t{:.4f}\n'
            .format(iter, 3, acc1_3, pre1_3, rec1_3, F1_3, acc2_3, pre2_3, rec2_3, F2_3))
    ...
    with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
        f.write('{}\t{}\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:4f}\t{:.4f}\n'
            .format(iter, 4, acc1_4, pre1_4, rec1_4, F1_4, acc2_4, pre2_4, rec2_4, F2_4))
...
print("Total_Test_ Accuracy: {:.4f}|acc1: {:.4f}|acc2: {:.4f}|pre1: {:.4f}|pre2: {:.4f}"
...
with open('{}/average_performance.tsv'.format(savepath), 'a') as f:
    f.write('-\t-\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\t{:.4f}\n'.format(
        sum(test_accs) / iterations, sum(ACC1) / iterations, sum(PRE1) / iterations, sum(REC1) / iterations, sum(F1) / iterations,\
                sum(ACC2) / iterations, sum(PRE2) /iterations, sum(REC2) / iterations,  sum(F2) / iterations))
writer.close()
